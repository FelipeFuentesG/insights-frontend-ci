...
type Modo = "programado" | "periodico";
type Repetir = "diario" | "semanal";

const DIAS: { label: string; num: number }[] = [
  { label: "L", num: 1 },
  { label: "M", num: 2 },
  { label: "X", num: 3 },
  { label: "J", num: 4 },
...
const HORAS_PERIODICO = [1, 2, 3, 4, 6, 8, 12];
...
function frescura(iso: string | null, ultimaFallida: boolean): { color: string; label: string } {
  const d = parseFecha(iso);
...
function estadoProgramacion(estado: string | null): string {
  if (estado === "ENABLED") return "Activa";
  if (estado === "PAUSED") return "Pausada";
  return "—";
}

function construirCron(p: Programacion): string {
...
function describir(p: Programacion): string {
...
function desdeCron(cron: string | null): Programacion {
