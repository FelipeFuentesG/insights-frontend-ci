...
const TABS = ["Indicadores", "Recencia", "Perfil"];
...
export default function Home() {
  ...
  const tenantMain =
    user?.rol === "admin_global_andesml" ? "AndesML" :
    user?.rol === "admin_retailer"       ? (user.retailerNombre ?? "Retailer") :
    (user?.marcaNombre ?? "Marca");

  const tenantSub = user?.rol === "admin_marca" ? (user.retailerNombre ?? null) : null;
      ...
      <main className="home-main">
        <header className="home-header">
          ...
          <div className="home-tenant-card">
            ...
            {tenantSub && <span className="home-tenant-sub">{tenantSub}</span>}
          </div>

          <div className="home-avatar-wrapper" ref={menuRef}>
            <div className="home-user-info">
              <div className="home-user-text">
                <span className="home-user-name">{nombre.split(" ").slice(0, 2).join(" ")}</span>
                <span className="home-user-rol">{user ? ROL_LABEL[user.rol] : ""}</span>
              </div>
              <button className="home-avatar" onClick={() => setMenuOpen(o => !o)} aria-label="Menú de usuario">
                {initials}
              </button>
            </div>
            {menuOpen && (
              <div className="home-dropdown">
                <button className="home-dropdown-item" onClick={handleLogout}>Cerrar sesión</button>
              </div>
            )}
          </div>
        </header>
        ...
        <div className="home-tabs">
          {TABS.map((tab, i) => (
            <button key={tab} className={`home-tab${activeTab === i ? " active" : ""}`} onClick={() => setActiveTab(i)}>
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="home-content">
          ...
          {activeTab === 1 && <div className="home-coming-soon">Sección Recencia en construcción.</div>}
